...
public class Main extends JavaPlugin implements Listener, GlobalHashMaps {
	...
	public static HashSet<String> validLogMaterials = new HashSet<>(
			Arrays.asList("LOG", "LOG_2", "LEGACY_LOG", "LEGACY_LOG_2", "ACACIA_LOG", "BIRCH_LOG", "DARK_OAK_LOG",
					"JUNGLE_LOG", "OAK_LOG", "SPRUCE_LOG", "CRIMSON_STEM", "WARPED_STEM"));
	...
	public void onEnable() {
		...
		for (Player online : Bukkit.getOnlinePlayers()) {
			try {
				getLogger().log(Level.INFO, online.getName().toString() + "'s talent: "
						+ talentHashMap.get(online.getUniqueId().toString()).toString());
			...
			try {
				getLogger().log(Level.INFO, online.getName().toString() + "'s faction: "
						+ factionHashMap.get(online.getUniqueId().toString()).toString());
			...
			try {
				getLogger().log(Level.INFO, online.getName().toString() + "'s rank: "
						+ playerRankHashMap.get(online.getUniqueId().toString()).toString());
...
//            					}
					...
					for (Entity entity : world.getEntities()) {
						if (entity.isValid()) {
							...
							if (entity.getCustomName() != null && entity.getCustomName().equals("Tracking Arrow")) {
								...
								if (!arrow.isOnGround()
										&& trackingBowTarget.get(shooter.getUniqueId().toString()) != null
										&& trackingBowTarget.get(shooter.getUniqueId().toString()).isValid()) {
									Entity entityTracked = trackingBowTarget.get(shooter.getUniqueId().toString());
									Vector newDir = entityTracked.getLocation().toVector()
							...
							if (entity instanceof Mob) {
								...
								if (mob.getTarget() != null && mob.getTarget() instanceof Player) {
									...
									if (talentHashMap.get(target.getUniqueId().toString()) != null
											&& talentHashMap.get(target.getUniqueId().toString()).equals("Enderian")) {
										mob.setTarget(null);
									}
								}
							}
						...
						if (talent != null) {
							if (talent.equals("Avian")) {
								for (int y = location.getBlockY() + 2; y < location.getBlockY() + 7; y++) {
									Location blockCheck = new Location(online.getWorld(), location.getBlockX(), y,
											location.getBlockZ());
			...
			public void run() {
				if (!Bukkit.getOnlinePlayers().isEmpty()) {
					...
					for (Player online : Bukkit.getOnlinePlayers()) {
						...
						// direction
						// if it returns false, the value for how long they haven't looked somewhere is
						// reset to 0 and the look direction value is set to the new value
						if (playerYawHashMap.get(online.getUniqueId().toString()) == online.getLocation().getYaw()) {
							playerUnchangedLookDirHashMap.put(online.getUniqueId().toString(),
									playerUnchangedLookDirHashMap.get(online.getUniqueId().toString()) + 1);
						} else {
						...
						if (!isBloodMoon && (FunctionsPlus.getTime(getServer(), online) % 24000) >= 13000
								&& ((FunctionsPlus.getTime(getServer(), online) % 24000) < 13020)) {
							if (((int) (Math.random() * 50)) == 7) {
								isBloodMoon = true;
								for (Player online_copy : Bukkit.getOnlinePlayers()) {
									online_copy.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands"
											+ ChatColor.DARK_RED + "+" + ChatColor.WHITE + "] " + ChatColor.RED
											+ "A blood moon is rising!");
								}
							}
						}
						if (isBloodMoon && (FunctionsPlus.getTime(getServer(), online) % 24000) >= 23000
								&& ((FunctionsPlus.getTime(getServer(), online) % 24000) < 23200)) {
							isBloodMoon = false;
							for (Player online_copy : Bukkit.getOnlinePlayers()) {
								online_copy.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands"
										+ ChatColor.DARK_RED + "+" + ChatColor.WHITE + "] " + ChatColor.RED
										+ "The blood moon sets, only to rise again soon.");
							}
						}
				...
				try {
					...
					if (chunkOwnerUUID.equals(p.getUniqueId())) {
						...
						if (playerDataConfig.contains("Users." + p.getUniqueId() + ".stats" + ".chunkClaims")) {
							playerDataConfig.set("Users." + p.getUniqueId() + ".stats" + ".chunkClaims",
									playerDataConfig.getInt("Users." + p.getUniqueId() + ".stats" + ".chunkClaims")
											- 1);
						}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		// command to reevaluate some user stats on the server, this is used to prevent
		// cheating (editing the server config files)
			...
			if (p.hasPermission("CommandsPlus.superuser")) {
				for (Player online : Bukkit.getOnlinePlayers()) {
					...
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.BLUE + "Reevaluating level of " + ChatColor.WHITE
							+ online.getName());

				}
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.BLUE + "Done");
			}
		...
		// -----------------------------------------------------------------------------------------------------------------------------------------------
		// System Commands
		if (label.equalsIgnoreCase("listplayers")
				&& playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")) {
			int i = 0;
			for (Player online : Bukkit.getOnlinePlayers()) {
		...
		if (label.equalsIgnoreCase("getplayerhead") && playerRankHashMap.get(p.getUniqueId().toString()) != null
				&& playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")) {
			if (args.length < 1) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Parameter error!");
				return true;
			}
		...
		if (label.equalsIgnoreCase("setrank") && playerRankHashMap.get(p.getUniqueId().toString()) != null
				&& playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")) {
			if (args.length < 2) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Parameter error!");
			...
			if (target == null) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Player does not exist!");
				return true;
			}
		...
		if (label.equalsIgnoreCase("vanillamode") && playerRankHashMap.get(p.getUniqueId().toString()) != null
				&& playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")) {
		...
		if (label.equalsIgnoreCase("c+help")) {
			...
			p.sendMessage(ChatColor.BLUE + "" + ChatColor.BOLD
					+ "https://docs.google.com/document/d/1o8gLXXOj9hEkDjY5OD3lujR_yNdKC9mI8v1sNKTa5SY/edit?usp=sharing");
		...
		if (label.equalsIgnoreCase("top")) {
			...
			if (!talent.equals("Terran") && (playerRankHashMap.get(p.getUniqueId().toString()) == null
					|| !playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser"))) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Only terrans can use /top!");
			...
			if (p.getWorld().getEnvironment() == Environment.NETHER) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You cannot use /top in the Nether!");
				return true;
			}
			...
			// Commands+ System Message
			p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
					+ ChatColor.WHITE + "] " + ChatColor.GREEN + "Teleported you to the top!");
		...
		if (label.equalsIgnoreCase("launch")) {
			...
			if (args.length == 0) {
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.AQUA + "" + ChatColor.BOLD + "Y E E T");
		...
		if (label.equalsIgnoreCase("cords")) {
			...
			if (args.length == 0) {
				...
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Your coordinates" + ": " + x + ", " + y + ", " + z);
				return true;
			...
			// send player's coordinates to everyone on the server
				...
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Sending your coordinates to everyone!");
				Location loc = p.getLocation();
				int x = loc.getBlockX();
				int y = loc.getBlockY();
				int z = loc.getBlockZ();
				for (Player online : Bukkit.getOnlinePlayers()) {
					online.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + p.getName() + "'s coordinates are: " + x + ", " + y + ", " + z);
				}
		...
		if (label.equalsIgnoreCase("creator")) {
			...
			p.sendMessage(
					ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+" + ChatColor.WHITE
							+ "] " + "The creator of this plugin is Joey Balardeta, otherwise known as aclownsquad.");
		...
		if (label.equalsIgnoreCase("dailyxp")) {
			...
			if (!(((String) ft.format(date)).equals((String) (playerDataConfig
					.getString("Users." + p.getUniqueId() + "." + "stats" + ".dailyXPLastClaimed"))))) {
				playerDataConfig.set("Users." + p.getUniqueId() + "." + "stats" + ".dailyXPLastClaimed",
						ft.format(date));
				saveConfig();
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Claimed daily XP");
				p.giveExpLevels(20);
			} else {
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Already claimed daily XP!");
			}
		...
		if (label.equalsIgnoreCase("wild")) {
			...
			if (p.getWorld().getEnvironment() == Environment.NETHER) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You cannot use /wild in the Nether!");
				return true;
			}

			if (!(p.getUniqueId().toString()).equals("b859c378-5ea4-47f8-b7e4-fc80bfc1b713")
					&& !canTpHashMap.get(p.getUniqueId().toString())) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You can only use /wild every 20 seconds!");
				return true;
			}
			...
			Location dest = new Location(Bukkit.getWorld("world"), destX,
					p.getWorld().getHighestBlockYAt(destX, destZ) - 1, destZ);
			...
			while (dest.getBlock().getType() == Material.WATER || dest.getBlock().getType() == Material.LAVA) {
				...
				dest = new Location(Bukkit.getWorld("world"), destX, p.getWorld().getHighestBlockYAt(destX, destZ) - 1,
						destZ);
			}
			int destXfinal = destX;
			int destZfinal = destZ;

			Location destFinal = new Location(Bukkit.getWorld("world"), destX,
					p.getWorld().getHighestBlockYAt(destX, destZ) + 1, destZ);
			...
			p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
					+ ChatColor.WHITE + "] " + "Starting Countdown");
			p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
					+ ChatColor.WHITE + "] " + ChatColor.BLUE + "3");

			int chunkGenRange = 1;

			for (int i = -chunkGenRange; i < chunkGenRange + 1; i++) {
				for (int j = -chunkGenRange; j < chunkGenRange + 1; j++) {
					Location chunkLoad = new Location(Bukkit.getWorld("world"), destX + (j * 16),
							p.getWorld().getHighestBlockYAt(destX, destZ), destZ + (i * 16));
			...
			for (int i = -chunkGenRange; i < chunkGenRange + 1; i++) {
				for (int j = -chunkGenRange; j < chunkGenRange + 1; j++) {
					Location chunkLoad = new Location(Bukkit.getWorld("world"), destXfinal + (j * 16),
							p.getWorld().getHighestBlockYAt(destXfinal, destZfinal), destZfinal + (i * 16));
			...
			scheduler.scheduleSyncDelayedTask(this, new Runnable() {
				public void run() {
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.BLUE + "2");
				}
			}, 20L);
			...
			scheduler.scheduleSyncDelayedTask(this, new Runnable() {
				public void run() {
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.BLUE + "1");
				}
			}, 40L);

			scheduler.scheduleSyncDelayedTask(this, new Runnable() {
				public void run() {
					if (!(currX == p.getLocation().getBlockX() && currY == p.getLocation().getBlockY()
							&& currZ == p.getLocation().getBlockZ())) {
						FunctionsPlus.playSound(p, "actionDenied");
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + ChatColor.RED + "You moved! Cancelling teleport!");
					} else {
						...
						// Commands+ System Message
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + "Teleported you into the wild!");
					}
				}
			}, 60L);
		...
		if (label.equalsIgnoreCase("waypoint")) {
			...
			if (args.length == 0) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Your waypoints:");
				p.sendMessage("--------------------------------");

				ConfigurationSection waypoints = playerDataConfig
						.getConfigurationSection("Users." + p.getUniqueId() + ".waypoints");
				for (String s : waypoints.getKeys(false)) {
					...
					if (!(wpX == 0 && wpY == 0 && wpZ == 0)) {
						TextComponent wayPointsMessage = new TextComponent(
								ChatColor.WHITE + ">> " + ChatColor.AQUA + wpName);
						wayPointsMessage
								.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/waypoint " + wpName));
						wayPointsMessage.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
								new ComponentBuilder("Teleport").color(ChatColor.GRAY).italic(true).create()));
			...
			if (args[0].equals("create")) {
				if (args.length == 1) {
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "You need to name your waypoint!");
					return true;
				}
					...
					for (String s : waypoints.getKeys(false)) {
						if (s.equalsIgnoreCase(wpName) && (wpX == 0 && wpY == 0 && wpZ == 0)) {
							p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
									...
									+ "You cannot make multiple waypoints of the same name!");
							return true;
						}
					}
				}
				...
				playerDataConfig.set("Users." + p.getUniqueId() + ".waypoints" + "." + wpName + ".X",
						p.getLocation().getBlockX());
				playerDataConfig.set("Users." + p.getUniqueId() + ".waypoints" + "." + wpName + ".Y",
						p.getLocation().getBlockY());
				playerDataConfig.set("Users." + p.getUniqueId() + ".waypoints" + "." + wpName + ".Z",
						p.getLocation().getBlockZ());

				playerDataConfig.set("Users." + p.getUniqueId() + ".stats" + ".numberOfWaypoints",
						0 + playerDataConfig.getInt("Users." + p.getUniqueId() + ".stats" + ".numberOfWaypoints") + 1);
				...
				p.sendMessage(
						ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+" + ChatColor.WHITE
								+ "] " + "Created waypoint " + ChatColor.AQUA + wpName + ChatColor.WHITE + "!");
			...
			if (args[0].equals("remove")) {
				if (args.length == 1) {
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED
							+ "You need to include the name of the waypoint that you want to remove!");
				}
				String wpName = args[1];
				for (int i = 2; i < args.length; i++) {
					wpName += " " + args[i];
				...
				if (!waypointExists) {
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "Waypoint does not exist!");
					return true;
				}
				...
				p.sendMessage(
						ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+" + ChatColor.WHITE
								+ "] " + "Removed waypoint " + ChatColor.AQUA + wpName + ChatColor.WHITE + "!");
			}
			...
			if (args[0].equals("update")) {
				...
				p.sendMessage(
						ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+" + ChatColor.WHITE
								+ "] " + "Updated waypoint " + ChatColor.AQUA + wpName + ChatColor.WHITE + "!");
			}
				...
				if (!(p.getUniqueId().toString()).equals("b859c378-5ea4-47f8-b7e4-fc80bfc1b713")
					...
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED
							+ "You can only teleport with /waypoint every 20 seconds!");
					return true;
				}
				...
				scheduler.scheduleSyncDelayedTask(this, new Runnable() {
					public void run() {
						if (!(currX == p.getLocation().getBlockX() && currY == p.getLocation().getBlockY()
							...
							// Commands+ System Message
							p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
									+ ChatColor.WHITE + "] " + "Teleported to " + ChatColor.AQUA + wpNameFinal
									+ ChatColor.WHITE + "!");
						}
					}
				}, 60L);
		...
		if (label.equalsIgnoreCase("claim")) {
			...
			if (p.getWorld().getEnvironment() == Environment.NETHER) {
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You cannot claim chunks in the Nether!");
				return true;
			}

			String UUID = chunkClaimDataConfig.getString("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX()
					+ ", Z: " + p.getLocation().getChunk().getZ() + ".belongsToUUID");

			String chunkName = chunkClaimDataConfig.getString("ClaimedChunks." + ".X: "
					+ p.getLocation().getChunk().getX() + ", Z: " + p.getLocation().getChunk().getZ() + ".appearsAs");
			String name = chunkClaimDataConfig.getString("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX()
					+ ", Z: " + p.getLocation().getChunk().getZ() + ".belongsToName");
			...
			if (UUID == null && chunkClaims < 400) {
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".belongsToUUID", "" + p.getUniqueId());
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".belongsToName", "" + p.getName());
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".appearsAs", "" + willAppearAs);

				chunkLocationsRAM
						.add("X: " + p.getLocation().getChunk().getX() + ", Z: " + p.getLocation().getChunk().getZ());
				...
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Claimed chunk!");
			} else if ((UUID == null && chunkClaims >= 400)) {
				FunctionsPlus.playSound(p, "actionDenied");
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED
						+ "You cannot claim any more chunks! Level up to get more chunk claims!");
			} else if (UUID.equals(p.getUniqueId().toString())) {
				FunctionsPlus.playSound(p, "actionDenied");
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You already own this chunk!");
			} else {
				// Commands+ System Message
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Cannot claim chunk! Chunk is owned by " + chunkName
						+ " (" + name + ")");
			}
		...
		if (label.equalsIgnoreCase("unclaim")) {
			...
			if (p.getWorld().getEnvironment() == Environment.NETHER) {
				...
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "You cannot unclaim chunks in the Nether!");
				return true;
			}
			...
			if (UUID.equals(p.getUniqueId().toString())) {
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".belongsToUUID", null);
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".belongsToName", null);
				chunkClaimDataConfig.set("ClaimedChunks." + ".X: " + p.getLocation().getChunk().getX() + ", Z: "
						+ p.getLocation().getChunk().getZ() + ".appearsAs", null);
				int chunkClaims = playerDataConfig.getInt("Users." + p.getUniqueId() + ".stats" + ".chunkClaims");
				chunkClaims--;

				try {
					int i = chunkLocationsRAM.indexOf(
							"X: " + p.getLocation().getChunk().getX() + ", Z: " + p.getLocation().getChunk().getZ());
					chunkLocationsRAM.remove(i);
					chunkNamesRAM.remove(i);
				} catch (Exception e) {
					getLogger().log(Level.SEVERE, "Chunk to unclaim not found in ArrayList!");
				...
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Unclaimed chunk!");
			} else if (UUID.equals(null)) {
				FunctionsPlus.playSound(p, "actionDenied");
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "This chunk has not been claimed!");
			} else {
				// Commands+ System Message
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Cannot unclaim chunk! Chunk is owned by "
						+ chunkName + " (" + name + ")");
			}
		...
		if (label.equalsIgnoreCase("whochunk")) {
			...
			if (name == null) {
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Chunk is unclaimed!");
			} else {
				// Commands+ System Message
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "Chunk is owned by " + chunkName + " (" + name + ")");
			}
		...
		if (label.equalsIgnoreCase("chunkmap")) {
			...
			if (p.getWorld().getEnvironment() == Environment.NETHER) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Chunkmap does not work in the Nether!");
				return true;
			}
		...
		if (label.equalsIgnoreCase("faction")) {
			...
			if (args.length == 0) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED
						+ "Invalid faction command! The available faction commands are:");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "/faction create <name> - " + ChatColor.YELLOW
						+ "creates a faction with the specified name.");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "/faction invite <player> - " + ChatColor.YELLOW
						+ "invites the specified player to your faction.");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "/faction remove <player> - " + ChatColor.YELLOW
						+ "removes the specified player from your faction.");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "/faction accept - " + ChatColor.YELLOW + "accept faction invite.");
				p.sendMessage(
						ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+" + ChatColor.WHITE
				...
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "/faction info <name> - " + ChatColor.YELLOW
						+ "gives info about the faction specified.");
				return true;
			}
			...
			if (args[0].equals("create")) {
				...
				if (playerDataConfig.getBoolean("Users." + p.getUniqueId() + ".stats" + ".inFaction")) {
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "You're already in a faction!");
				...
				try {
					...
					for (String s : factions.getKeys(false)) {
						if (s.equals(factionName)) {
							FunctionsPlus.playSound(p, "actionDenied");
							p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
									+ ChatColor.WHITE + "] " + ChatColor.RED + "Faction alrady exists!");
							return true;
						}
					}
				} catch (Exception e) {
				...
				if (factionName.length() > 20) {
					...
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED
							+ "Faction name cannot be longer than 20 characters!");
				...
				if (factionDataConfig.getString("Factions." + factionName + ".ownerUUID") == null) {
					...
					factionDataConfig.set("Factions." + factionName + ".members." + p.getUniqueId() + ".name",
							"" + p.getName());
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.WHITE + "Created faction " + factionName);
					...
					FunctionsPlus.playSound(p, "actionDenied");
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "Faction name is already in use!");
			...
			if (args[0].equals("invite")) {
				...
				if (factionDataConfig.getString("Factions." + playerFaction + ".stats" + ".ownerUUID")
					...
					if (playerDataConfig
							.getBoolean("Users." + playerInvited.getUniqueId() + ".stats" + ".inFaction") != true) {
						playerDataConfig.set("Users." + playerInvited.getUniqueId() + ".stats" + ".invitedToFaction",
								true);
						factionDataConfig.set(
								"Factions." + playerFaction + ".members." + playerInvited.getUniqueId() + ".name",
								"" + playerInvited.getName());
						factionDataConfig.set(
								"Factions." + playerFaction + ".members." + playerInvited.getUniqueId() + ".rank",
								"invited");
						...
						// Commands+ System Message
						playerInvited.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands"
								+ ChatColor.DARK_RED + "+" + ChatColor.WHITE + "] " + "You've been invited to "
								+ ChatColor.AQUA + playerFaction + ChatColor.WHITE + " by " + p.getName()
								+ "! Type /faction accept to join!");
						// Commands+ System Message
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.AQUA + playerFaction + ChatColor.WHITE + "] "
								+ "Invited " + ChatColor.GREEN + playerInvited.getName() + ChatColor.WHITE + " to "
								+ ChatColor.AQUA + playerFaction + ChatColor.WHITE + "!");
					} else {
						FunctionsPlus.playSound(p, "actionDenied");
						// Commands+ System Message
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + "Player is in another faction!");
					}
			...
			if (args[0].equals("accept")) {
				...
				ConfigurationSection factions = factionDataConfig.getConfigurationSection("Factions.");
				for (String s : factions.getKeys(false)) {
					if (factions.get(s + ".members." + p.getUniqueId() + ".rank") != null
							&& factions.get(s + ".members." + p.getUniqueId() + ".rank").equals("invited")) {
						factionName = s;
					}
				}
				if (!factionName.equals("")) {
					factionDataConfig.set("Factions." + factionName + ".stats" + ".memberCount",
							factionDataConfig.getInt("Factions." + factionName + ".stats" + ".memberCount") + 1);
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + "Joined faction " + ChatColor.AQUA + factionName
							+ ChatColor.WHITE + "!");
					for (Player online : Bukkit.getOnlinePlayers()) {
						if (playerDataConfig
								.getBoolean("Users." + online.getUniqueId() + ".stats" + ".inFaction") == true
								&& playerDataConfig.getString("Users." + online.getUniqueId() + ".stats" + ".faction")
										.equals(factionName)) {
							online.sendMessage(ChatColor.WHITE + "[" + ChatColor.AQUA + factionName + ChatColor.WHITE
									+ "] " + ChatColor.GREEN + p.getName() + ChatColor.WHITE + " joined the faction!");
						}
					}
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + "You haven't been invited to a faction yet!");
				}
			...
			if (args[0].equals("decline")) {
				...
				if (!factionName.equals("")) {
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + "Declined invite from faction " + ChatColor.AQUA + factionName
							+ ChatColor.WHITE + "!");
				} else {
					FunctionsPlus.playSound(p, "actionDenied");
					// Commands+ System Message
			...
			if (args[0].equals("remove")) {
				if (factionDataConfig.getString("Factions." + playerFaction + ".stats" + ".ownerUUID")
					...
					playerDataConfig.set("Users." + playerRemoved.getUniqueId() + ".stats" + ".invitedToFaction",
							false);
					playerDataConfig.set("Users." + playerRemoved.getUniqueId() + ".stats" + ".faction", "");

					factionDataConfig.set(
							"Factions." + playerFaction + ".members." + playerRemoved.getUniqueId() + ".rank", "none");
					factionDataConfig.set("Factions." + playerFaction + ".stats" + ".memberCount",
							factionDataConfig.getInt("Factions." + playerFaction + ".stats" + ".memberCount") - 1);
					...
					playerRemoved.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED
							+ "+" + ChatColor.WHITE + "] " + "You've been removed from " + ChatColor.AQUA
							+ playerFaction + ChatColor.WHITE + "!");
					for (Player online : Bukkit.getOnlinePlayers()) {
						if (playerDataConfig
								.getBoolean("Users." + online.getUniqueId() + ".stats" + ".inFaction") != false
								&& playerDataConfig.getString("Users." + online.getUniqueId() + ".stats" + ".faction")
										.equals(playerFaction)) {
							online.sendMessage(ChatColor.WHITE + "[" + ChatColor.AQUA + playerFaction + ChatColor.WHITE
									+ "] " + ChatColor.GREEN + playerName + ChatColor.WHITE
									+ " was kicked from the faction!");
						}
					}
				}
			...
			if (args[0].equals("leave")) {
				if (playerDataConfig.getBoolean("Users." + p.getUniqueId() + ".stats" + ".inFaction")) {
					...
					if (factionDataConfig.get("Factions." + playerFaction + ".stats" + ".ownerUUID")
							.equals(p.getUniqueId().toString())) {
						FunctionsPlus.playSound(p, "actionDenied");
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + ChatColor.RED
								+ "You must promote someone before leaving the faction! If you want to disband your faction you can do so by using the '/faction disband' command.");
						return true;
					}
					...
					factionDataConfig.set("Factions." + playerFaction + ".stats" + ".memberCount",
							factionDataConfig.getInt("Factions." + playerFaction + ".stats" + ".memberCount") - 1);
					factionDataConfig.set("Factions." + playerFaction + ".members." + p.getUniqueId() + ".rank",
							"none");
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + "You've left " + ChatColor.AQUA + playerFaction + ChatColor.WHITE
							+ "!");
					...
					for (Player online : Bukkit.getOnlinePlayers()) {
						if (playerDataConfig
								...
								&& playerDataConfig.getString("Users." + online.getUniqueId() + ".stats" + ".faction")
										.equals(playerFaction)) {
							online.sendMessage(ChatColor.WHITE + "[" + ChatColor.AQUA + playerFaction + ChatColor.WHITE
									+ "] " + ChatColor.GREEN + p.getName() + ChatColor.WHITE
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "You're not in a faction!");
				}
			...
			if (args[0].equals("info")) {
				String factionName = "";
				if (args.length == 1
						&& playerDataConfig.getBoolean("Users." + p.getUniqueId() + ".stats" + ".inFaction") != true) {
					FunctionsPlus.playSound(p, "actionDenied");
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "Type in a faction name to see stats!");
					return true;
				}
				if (args.length == 1) {
				...
				if (!factionDataConfig.contains("Factions." + factionName)) {
					...
					// Commands+ System Message
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "Faction does not exist!");
					return true;
				}

				ConfigurationSection faction = factionDataConfig
						.getConfigurationSection("Factions." + factionName + ".members");
				String factionMembers = "";

				for (String s : faction.getKeys(false)) {
					if (!(faction.get(s + ".rank")).equals(null) && !(faction.get(s + ".rank")).equals("invited")
							&& !(faction.get(s + ".rank")).equals("none")) {
			...
			if (args[0].equals("msg")) {
				...
				if (args.length == 1) {
					FunctionsPlus.playSound(p, "actionDenied");
					p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
							+ ChatColor.WHITE + "] " + ChatColor.RED + "You need to enter a message!");
					return true;
				} else {
					msg = args[1];
					for (int i = 2; i < args.length; i++) {
				...
				for (Player online : Bukkit.getOnlinePlayers()) {
					if (factionHashMap.get(p.getUniqueId().toString()) != null
							&& factionHashMap.get(p.getUniqueId().toString())
									.equals(factionHashMap.get(online.getUniqueId().toString()))) {
						online.sendMessage(ChatColor.WHITE + "[" + ChatColor.AQUA + playerFaction + ChatColor.WHITE
								+ "] " + ChatColor.GREEN + p.getName() + ChatColor.WHITE + ": " + msg);
					}
				}
			}
			...
			if (args[0].equals("promote")) {
				if (factionDataConfig.getString("Factions." + playerFaction + ".stats" + ".ownerUUID")
					...
					if (!factionHashMap.get(p.getUniqueId().toString())
							.equals(factionHashMap.get(playerPromoted.getUniqueId().toString()))) {
						FunctionsPlus.playSound(p, "actionDenied");
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + ChatColor.RED + "That player is not in your faction!");
						return true;
					}

					if (factionDataConfig
							.get("Factions." + playerFaction + ".members." + playerPromoted.getUniqueId() + ".rank")
							.equals("Member")) {
						factionDataConfig.set(
								"Factions." + playerFaction + ".members." + playerPromoted.getUniqueId() + ".rank",
								"Admin");
					}
					...
					if (factionDataConfig
						...
						factionDataConfig.set("Factions." + playerFaction + ".members." + p.getUniqueId() + ".rank",
								"Admin");
						factionDataConfig.set("Factions." + playerFaction + ".stats" + ".ownerUUID",
								playerPromoted.getUniqueId());
						factionDataConfig.set("Factions." + playerFaction + ".stats" + ".ownerName",
								playerPromoted.getName());
					}
			...
			if (args[0].equals("demote")) {
				if (factionDataConfig.getString("Factions." + playerFaction + ".stats" + ".ownerUUID")
					...
					if (!factionHashMap.get(p.getUniqueId().toString())
							.equals(factionHashMap.get(playerDemoted.getUniqueId().toString()))) {
						FunctionsPlus.playSound(p, "actionDenied");
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + ChatColor.RED + "That player is not in your faction!");
						return true;
					...
					if (factionDataConfig
							.get("Factions." + playerFaction + ".members." + playerDemoted.getUniqueId() + ".rank")
							.equals("Member")) {
						FunctionsPlus.playSound(p, "actionDenied");
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + ChatColor.RED
								+ "Cannot demote a player with the rank of member! If you want to remove this player from your faction, you can do so with the '/faction remove <player>' command!");
						return true;
					}
					...
					if (factionDataConfig
							...
							.equals("Admin")) {
						factionDataConfig.set(
								"Factions." + playerFaction + ".members." + playerDemoted.getUniqueId() + ".rank",
								"Member");
					}
				...
				for (Player online : Bukkit.getOnlinePlayers()) {
					if (factionHashMap.get(p.getUniqueId().toString()) != null
							&& factionHashMap.get(p.getUniqueId().toString())
									.equals(factionHashMap.get(online.getUniqueId().toString()))) {
						p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
								+ ChatColor.WHITE + "] " + "Faction has been disbanded!");
		...
		if (label.equalsIgnoreCase("menu")) {
			...
			currentOpenInventory.put(p.getUniqueId().toString(), "Master Menu");
		...
		if (label.equalsIgnoreCase("bonkstick")
				&& (playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")
						|| playerRankHashMap.get(p.getUniqueId().toString()).equals("Sheriff"))) {
		...
		if (label.equalsIgnoreCase("yeehaw")) {
			...
			if (!(playerRankHashMap.get(p.getUniqueId().toString()).equals("Superuser")
					|| playerRankHashMap.get(p.getUniqueId().toString()).equals("Sheriff"))) {
				FunctionsPlus.playSound(p, "actionDenied");
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + ChatColor.RED + "Only the sheriff can use this command!");
				return true;
			}
			...
			Horse horse = (Horse) p.getWorld()
					.spawn(p.getLocation().add(Math.cos(FunctionsPlus.getPlayerDirectionFloat(p)) * 2, 0,
							Math.sin(FunctionsPlus.getPlayerDirectionFloat(p)) * 2), Horse.class);
		...
		if (label.equalsIgnoreCase("cobble")) {
			if (playerRankHashMap.get(p.getUniqueId().toString()).equalsIgnoreCase("COBBLE MAN")) {
				p.sendMessage(ChatColor.WHITE + "[" + ChatColor.RED + "Commands" + ChatColor.DARK_RED + "+"
						+ ChatColor.WHITE + "] " + "mmmmmmmmmmmmmmmm Cobble");
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (label.equalsIgnoreCase("faction")) {
			...
			for (String rank : rankList) {
				if (args.length == 2 && rank.toString().startsWith(args[1])) {
